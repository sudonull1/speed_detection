...
while(cap.isOpened()):
    ...
    if ret==True:
        ...
        for cnt in countours0:
            ...
            if area>areaTH:
                ...
                location2 = [cx, cy, x+w, y+h]
                sx = estimateSpeed(location1, location2, fps)
                if sx > 60:
                    count+=1
                
                cv2.putText(img, str(int(sx)) + " km/hr", (int(x + w/2), int(y-5)),cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 255, 255), 2)
        for i in cars:
    ...
    final_ls = list(set(ls))

    print(final_ls)
    df = pd.DataFrame(final_ls)
    df.to_csv('name')
    print("number of vehicles going wrong way is: ", len(final_ls))
    print("number of vehicle going overspeed are : ", count//fps)
